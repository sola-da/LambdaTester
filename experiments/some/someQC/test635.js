





var callbackArguments = [];
var argument1 = function() {
 callbackArguments.push(arguments) 
 return false; };
var argument2 = [460,969];
var argument3 = function() {
 callbackArguments.push(arguments) 
 return 83.95592079585987; };
var argument4 = r_1;
var argument5 = {"59":242,"126":6.520376400999481e+307,"969":"sqn","":"","1.3112379873591054e+308":","};
var argument6 = function() {
 callbackArguments.push(arguments) 
 return [0,0.22913438829923582,null,"Æ,",false,-23.97842008131761,true]; };
var argument7 = function() {
 callbackArguments.push(arguments) 
 return undefined; };
var argument8 = r_0;
var base_0 = [82,100,0,25,893,655,403,5e-324,705,460]
var r_0= undefined
try {
r_0 = base_0.some(argument1,argument2)
}
catch(e) {
r_0= "Error"
}
var base_1 = [82,100,0,25,893,655,403,5e-324,705,460]
var r_1= undefined
try {
r_1 = base_1.some(argument3,argument4,argument5)
}
catch(e) {
r_1= "Error"
}
var base_2 = [82,100,0,25,893,655,403,5e-324,705,460]
var r_2= undefined
try {
r_2 = base_2.some(argument6)
}
catch(e) {
r_2= "Error"
}
var base_3 = [82,100,0,25,893,655,403,5e-324,705,460]
var r_3= undefined
try {
r_3 = base_3.some(argument7,argument8)
}
catch(e) {
r_3= "Error"
}
function serialize(array){
return array.map(function(a){
if (a === null || a == undefined) return a;
var name = a.constructor.name;
if (name==='Object' || name=='Boolean'|| name=='Array'||name=='Number'||name=='String')
return JSON.stringify(a);
return name;
 });
}
setTimeout(function(){
require("fs").writeFileSync("./experiments/some/someQC/test635.json",JSON.stringify({"baseObjects":serialize([base_0,base_1,base_2,base_3]),"returnObjects":serialize([r_0,r_1,r_2,r_3]),"callbackArgs":callbackArguments}))
},300)