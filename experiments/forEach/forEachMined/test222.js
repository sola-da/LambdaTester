





var callbackArguments = [];
var argument1 = function (url) {
 callbackArguments.push(arguments) 

    output += options.fn({ url: url });
};
var argument2 = function (axisCategData) {
 callbackArguments.push(arguments) 

    var categData = data.child(axisCategData.key), group = categData && categData.child(seriesKey), scene = new pvc.visual.Scene(seriesScene, { source: group }), categVar = scene.vars.category = pvc_ValueLabelVar.fromComplex(categData);
    categVar.group = categData;
    valueVarHelper.onNewScene(scene, !0);
    colorVarHelper.onNewScene(scene, !0);
};
var argument3 = "n]";
var argument4 = null;
var argument5 = function (leaf) {
 callbackArguments.push(arguments) 

    self.get(id, {
        rev: leaf,
        revs: opts.revs,
        attachments: opts.attachments
    }, function (err, doc) {
        if (!err) {
            result.push({ ok: doc });
        } else {
            result.push({ missing: leaf });
        }
        count--;
        if (!count) {
            callback(null, result);
        }
    });
};
var argument6 = function (m) {
 callbackArguments.push(arguments) 

    if (m.message === 'attributes' && m.attributeName === 'class') {
        classChanged(m);
    } else if (m.message === 'attributes' && m.attributeName === 'style') {
        styleChanged(m);
    }
    if (m.message === 'childList' && m.addedNodes.length > 0) {
        nodesAdded(m);
    }
    if (m.message === 'childList' && m.removedNodes.length > 0) {
        nodesRemoved(m);
    }
};
var base_0 = ["V",655,969,607,"o",969,969,595,126]
var r_0= undefined
try {
r_0 = base_0.forEach(argument1)
}
catch(e) {
r_0= "Error"
}
var base_1 = ["V",655,969,607,"o",969,969,595,126]
var r_1= undefined
try {
r_1 = base_1.forEach(argument2,argument3,argument4)
}
catch(e) {
r_1= "Error"
}
var base_2 = ["V",655,969,607,"o",969,969,595,126]
var r_2= undefined
try {
r_2 = base_2.forEach(argument5)
}
catch(e) {
r_2= "Error"
}
var base_3 = ["V",655,969,607,"o",969,969,595,126]
var r_3= undefined
try {
r_3 = base_3.forEach(argument6)
}
catch(e) {
r_3= "Error"
}
function serialize(array){
return array.map(function(a){
if (a === null || a == undefined) return a;
var name = a.constructor.name;
if (name==='Object' || name=='Boolean'|| name=='Array'||name=='Number'||name=='String')
return JSON.stringify(a);
return name;
 });
}
setTimeout(function(){
require("fs").writeFileSync("./experiments/forEach/forEachMined/test222.json",JSON.stringify({"baseObjects":serialize([base_0,base_1,base_2,base_3]),"returnObjects":serialize([r_0,r_1,r_2,r_3]),"callbackArgs":callbackArguments}))
},300)