





var callbackArguments = [];
var argument1 = function (mode) {
 callbackArguments.push(arguments) 

    if (channel.removeMode[mode])
        channel.removeMode[mode].apply(channel, [
            user,
            arg
        ]);
};
var argument2 = function (ref) {
 callbackArguments.push(arguments) 

    if (ref instanceof AST_Continue) {
        ref = ref.label.start;
        croak('Continue label `' + label.name + '` refers to non-IterationStatement.', ref.line, ref.col, ref.pos);
    }
};
var argument3 = false;
var argument4 = [213];
var argument5 = function (store) {
 callbackArguments.push(arguments) 

    tx.executeSql('DROP TABLE IF EXISTS ' + store, []);
};
var argument6 = false;
var argument7 = function (ct) {
 callbackArguments.push(arguments) 

    var complexType = parseCsdlComplexType(ct, schema, metadataStore);
};
var argument8 = r_3;
var argument9 = r_2;
var base_0 = [618,607,618,49]
var r_0= undefined
try {
r_0 = base_0.forEach(argument1)
}
catch(e) {
r_0= "Error"
}
var base_1 = [618,607,618,49]
var r_1= undefined
try {
r_1 = base_1.forEach(argument2,argument3,argument4)
}
catch(e) {
r_1= "Error"
}
var base_2 = [618,607,618,49]
var r_2= undefined
try {
r_2 = base_2.forEach(argument5,argument6)
}
catch(e) {
r_2= "Error"
}
var base_3 = [618,607,618,49]
var r_3= undefined
try {
r_3 = base_3.forEach(argument7,argument8,argument9)
}
catch(e) {
r_3= "Error"
}
function serialize(array){
return array.map(function(a){
if (a === null || a == undefined) return a;
var name = a.constructor.name;
if (name==='Object' || name=='Boolean'|| name=='Array'||name=='Number'||name=='String')
return JSON.stringify(a);
return name;
 });
}
setTimeout(function(){
require("fs").writeFileSync("./experiments/forEach/forEachMined/test83.json",JSON.stringify({"baseObjects":serialize([base_0,base_1,base_2,base_3]),"returnObjects":serialize([r_0,r_1,r_2,r_3]),"callbackArgs":callbackArguments}))
},300)