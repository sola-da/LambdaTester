





var callbackArguments = [];
var argument1 = function (handler) {
 callbackArguments.push(arguments) 

    handler.func.apply(null, args);
};
var argument2 = ["D","cX]o_",":","a","r5","<","=","^z|1","_"];
var argument3 = function (plugin) {
 callbackArguments.push(arguments) 

    return plugin.destroy();
};
var argument4 = null;
var argument5 = function (fn, index) {
 callbackArguments.push(arguments) 

    if (fn.id === deferId)
        fnIndex = index;
};
var argument6 = function Pending_eachObserver(observer) {
 callbackArguments.push(arguments) 

    asap(function Pending_setEstimate_eachObserver_task() {
        observer.call(void 0, estimate);
    });
};
var base_0 = [100,969,0,893,823,893]
var r_0= undefined
try {
r_0 = base_0.forEach(argument1,argument2)
}
catch(e) {
r_0= "Error"
}
var base_1 = [100,969,0,893,823,893]
var r_1= undefined
try {
r_1 = base_1.forEach(argument3,argument4)
}
catch(e) {
r_1= "Error"
}
var base_2 = [100,969,0,893,823,893]
var r_2= undefined
try {
r_2 = base_2.forEach(argument5)
}
catch(e) {
r_2= "Error"
}
var base_3 = [100,969,0,893,823,893]
var r_3= undefined
try {
r_3 = base_3.forEach(argument6)
}
catch(e) {
r_3= "Error"
}
function serialize(array){
return array.map(function(a){
if (a === null || a == undefined) return a;
var name = a.constructor.name;
if (name==='Object' || name=='Boolean'|| name=='Array'||name=='Number'||name=='String')
return JSON.stringify(a);
return name;
 });
}
setTimeout(function(){
require("fs").writeFileSync("./experiments/forEach/forEachMined/test416.json",JSON.stringify({"baseObjects":serialize([base_0,base_1,base_2,base_3]),"returnObjects":serialize([r_0,r_1,r_2,r_3]),"callbackArgs":callbackArguments}))
},300)